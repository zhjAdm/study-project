<template>
    <div>
        <el-card v-for="(group,gin) in groups" :key="gin" style="margin-bottom: 10px;" shadow="never">
            <div slot="header">
                {{group.groupName}}
            ...
            <div>
                <el-card v-for="(tab,tabin) in group.tableLists" :key="tabin" shadow="hover" style="marin-bottom:20px;">
        ...
        <conference-record ref="1" v-on:reload="reload"></conference-record>
        <first-report ref="0" v-on:reload="reload"></first-report>
        <first-report-evaluate ref="firstReportEvaluate"></first-report-evaluate>
    </div>
</template>
